...
- **Verification detail:** **Direct contradiction found in this pass**: the migration file's own header comment states *"NOT YET APPLIED TO THE LIVE DATABASE... database changes require explicit human review before being applied"* and *"That edge-function change is a follow-up once this migration is reviewed and applied — it is not wired in yet"* — but `SECURITY_RELEASE_GATE.md` §F and the git log entry `docs(security): record both migrations as applied to production, verified` both claim this exact migration (`20260818120000`) **was** applied to production on 2026-08-18 and verified by direct query. One of these two documents is stale. If the migration file's in-repo comment is the outdated one, the guard is live; if `SECURITY_RELEASE_GATE.md` is optimistic/premature, real money could still be released twice via `thrivefund-release-milestone`. This needs a direct, live-database check (query `information_schema.tables` for `thrivefund_milestone_releases`), not a re-read of either doc, to resolve.
- **Evidence required:** Run a read-only query against the production database to confirm whether `thrivefund_milestone_releases` actually exists, then reconcile/update whichever doc is stale.
- **Recommended action:** **Treat as unresolved until directly verified against production** — this is a real-money double-spend risk, the highest-stakes discrepancy found in this entire catalog. Recommend Noé personally confirm table existence before the 26 Aug due date.
...
- **Verification detail:** `GLOBAL_UX_QA.md` predates this Trello card's creation (18 Aug) by 3 days (dated 15 Aug) — it may already answer several of these criteria but wasn't opened in this pass to confirm currency.
- **Evidence required:** Read `GLOBAL_UX_QA.md` and `UX_NAVIGATION_AUDIT.md` to see how much of this checklist they already cover; then a fresh visual sweep for anything shipped since 15 Aug.
- **Recommended action:** Start from the existing UX QA docs rather than a blind re-sweep — likely largely done given two related Done-card claims, but needs a final confirmation pass since the docs predate the card.
...
- **Verification detail:** Did not check all 20 files individually, only 3; a full pass would need to confirm none of the remaining 17 render "ThriveIN" in visible UI copy.
- **Evidence required:** A `grep -i thrivein` sweep restricted to JSX text nodes / string literals rendered to the DOM (not storage keys or comments), across all 20 hits.
- **Recommended action:** Low risk based on the sample checked, but worth a 10-minute full sweep of the remaining 17 files before checking this box off, since it's explicitly called out as its own acceptance criterion.
