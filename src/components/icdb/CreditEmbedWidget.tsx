...
export const CreditEmbedWidget = ({ creatorId, fullName }: CreditEmbedWidgetProps) => {
  ...
  <img src="${baseUrl}/api/badge/${creatorId}" alt="${fullName} - Kretopia Verified" height="28" />
</a>`,
    card: `<iframe 
...
></iframe>`,
  ...
  const markdownBadge = `[![${fullName} - Kretopia Verified](${baseUrl}/api/badge/${creatorId})](${baseUrl}/epk/${creatorId})`;
          ...
          {style === "card" && (
            <div className="w-full max-w-[300px] p-3 rounded-lg border bg-background text-center">
              ...
              <Badge variant="outline" className="text-[9px] mt-2 gap-0.5">
                <ShieldCheck className="h-2 w-2" /> Kretopia Verified
              </Badge>
            </div>
          )}
          {style === "timeline" && (
