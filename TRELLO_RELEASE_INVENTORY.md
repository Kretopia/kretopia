...
| 2.3 Review Passport as the core product | List 2 — P0 Core Product Loop | 🟡 PARTIALLY_VERIFIED (4/6 confirmed; Bug A fixed pending deploy, Bug B open) | Jeff | 22 Aug, 02:00 |
...
- **Status:** 🟡 PARTIALLY_VERIFIED — 4/6 confirmed live; Bug A fixed 2026-08-20 (pending deploy), Bug B and the visibility gap still open
...
- **Evidence required:** Once this branch's edge functions deploy, re-test live: dismiss/clear the existing stale "0%" proposal, trigger a fresh `surface-agent-watch` run, and confirm the new nudge (if any) agrees with "Passport Strength." Consolidate the 6 Kreto Action Center rows down to the 2-3 real underlying actions (Bug B, not yet fixed). Confirm whether the "ThriveXchange Bali 2022 Promo Video" entries are one record or a genuine duplicate needing a merge/dedup pass. Decide whether whole-Passport visibility settings are in scope for this release or explicitly deferred post-beta.
- **Recommended action:** Bug A's code fix is done; re-verify live once deployed. Bug B and the visibility gap are real but lower-severity UX/scope items suitable for a fast-follow rather than blocking release.
