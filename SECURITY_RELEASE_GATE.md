...
| No critical RLS finding open on payment/wallet tables | **YES, as of 2026-08-24.** KP-01 and KP-02 (§H) are closed — confirmed by live query (`has_table_privilege`/`has_column_privilege` against production, 20/20 checks passing; see `WALLET_SECURITY_VERIFICATION_REPORT.md`). The fix that actually landed came from a parallel Lovable AI session's migration (table-level `REVOKE` across `wallets`, `creator_wallets`, and three sibling tables, plus a full `profiles` column allow-list) reconciled into this branch via merge — broader than this session's original column-level-only migration, which it superseded. Full account in `PRIVILEGE_DRIFT_INVESTIGATION.md`. |

**Verdict: CLEARED for KP-01/KP-02**, reversing the prior NOT CLEARED verdict in this section. Two silent reversals happened between that verdict and this one — see `WALLET_SECURITY_VERIFICATION_REPORT.md` for the full timeline — so this clearance rests on a live, direct privilege-function query, not a reported-successful migration statement, which is exactly the distinction that caught the two earlier false "fixed" conclusions. Not yet done: an actual negative-test `PATCH` request per `KREPAY_WALLET_NEGATIVE_TEST_MATRIX.md`, and a time-delayed re-check, both still recommended before treating this as permanently settled. The hire-notification fix (`accept_application_and_create_studio` plus this session's supplement) is also applied via the same merge but has not had the equivalent live re-verification pass yet. No secret rotation was performed or required; none was discovered in tracked source.
