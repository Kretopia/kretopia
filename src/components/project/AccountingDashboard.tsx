...
interface AccountingDashboardProps {
  projectId?: string; // Optional — if not provided, shows all user data
}
...
export function AccountingDashboard({ projectId }: AccountingDashboardProps) {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return invoices.filter(inv =>
      isWithinInterval(new Date(inv.created_at), { start: range.start, end: range.end })
    );
  }, [invoices, period]);
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter(p =>
      isWithinInterval(new Date(p.created_at), { start: range.start, end: range.end })
    );
  }, [payments, period]);
  ...
  const stats = useMemo(() => {
    ...
    return { totalInvoiced, totalCollected, overdueAmount, pendingAmount, received, sent, collectionRate, overdueCount: overdueInvoices.length, pendingCount: pendingInvoices.length, paidCount: paidInvoices.length, totalInvoiceCount: filteredInvoices.length };
  ...
  const exportCSV = () => {
    const rows = [
      ["Date", "Type", "Description", "Invoice #", "Status", "Amount", "Currency"],
      ...filteredInvoices.map(inv => [
        format(new Date(inv.created_at), "yyyy-MM-dd"),
        "Invoice",
        inv.recipient_name || "Client",
        inv.invoice_number,
        inv.status,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-3">
            ...
            <p className="text-[10px] text-muted-foreground">{stats.paidCount} paid • {stats.collectionRate.toFixed(0)}% rate</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-3">
            ...
            <p className="text-[10px] text-muted-foreground">{stats.pendingCount} pending</p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">Invoices & payments combined</CardDescription>
        </CardHeader>
              ...
              <TableBody>
                ...
                {[
                  ...
                  ...filteredPayments.map(p => ({
                    ...
                    currency: p.currency,
                    isIncome: p.type === "payment_received",
                  }))
                ]
                    ...
                    <TableRow key={i}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={`text-[9px] ${
                          tx.status === "paid" || tx.status === "completed" ? "text-green-600" :
