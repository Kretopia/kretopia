...
const TYPE_CONFIG: Record<string, { label: string; color: string; bgColor: string; icon: typeof Briefcase }> = {
  ...
  barter: { label: "Barter", color: "text-indigo-700", bgColor: "bg-primary/10 border-primary/20", icon: ArrowRightLeft },
};
        ...
        {isBarter && (opp.barter_offering || opp.barter_requesting) && (
          <div className="px-3 py-2 bg-primary/5 border-b border-primary/10">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-indigo-700 dark:text-primary truncate">
          ...
          <div className="flex gap-3">
            {opp.image_url ? (
              ...
              <div className={`shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-lg flex items-center justify-center ${
                isBarter ? "bg-primary/10" : "bg-muted"
              }`}>
                <TypeIcon className={`h-5 w-5 ${isBarter ? "text-primary" : "text-muted-foreground"}`} />
              </div>
            )}
