...
import {
  Bug, Lightbulb, Palette, MessageCircle, CheckCircle2,
  Clock, ArrowRight, X, Loader2, ChevronDown
} from "lucide-react";
import {
...
export function FeedbackTab() {
  ...
  const saveNotes = async (id: string) => {
    setSaving(true);
    const { error } = await supabase.from("feedback").update({ admin_notes: adminNotes }).eq("id", id);
    if (!error) {
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => updateStatus(item.id, "implemented")}>
                          <CheckCircle2 className="h-4 w-4 mr-2" /> Implemented
                        </DropdownMenuItem>
            ...
            <CardContent className="flex-1 overflow-auto space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => updateStatus(selectedItem.id, "wont_fix")} disabled={saving}>
                    Won't Fix
                  </Button>
                </div>
              </div>
              ...
              <div>
                ...
                <Textarea value={adminNotes} onChange={e => setAdminNotes(e.target.value)} placeholder="Add internal notes..." className="text-sm" rows={3} />
                <Button size="sm" className="mt-2" onClick={() => saveNotes(selectedItem.id)} disabled={saving}>
