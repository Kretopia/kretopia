# Kretopia Investor Deck
## Pre-Seed Round: $350K+

**Prepared:** February 2026
**Founder:** Ethan Auguste
**Website:** kretopia.com

---

## Slide 1: Title / Hook

### Kretopia
# The Operating System for Creative Collaboration

**13 years of community building. Now going digital.**

AI-powered matching • Verified portfolios • End-to-end payments • Global creator network

*Raising $350K+ Pre-Seed at $1.75M-$2M pre-money valuation*

---

## Slide 2: The Problem

### Finding Creative Collaborators is Broken

**The Pain:**
- 🔍 **Scattered Discovery** - Creators waste 10+ hours/week scrolling Facebook groups, WhatsApp threads, and LinkedIn
- ❌ **No Verification** - Can't verify skills, credits, or work quality before committing
- 💸 **Payment Friction** - No integrated way to scope, track, and get paid for projects
- 🌍 **Industry Silos** - Musicians can't find videographers; designers can't find copywriters

**Market Reality:**
> "I spend more time finding the right collaborator than actually creating."
> — Common feedback from 50+ creator interviews

**The Result:** 
- Projects delayed or abandoned
- Mismatched collaborations
- Payment disputes and ghosting

---

## Slide 3: The Solution

### Kretopia: Find → Verify → Work → Get Paid

**One platform for the entire creative collaboration lifecycle:**

| Phase | What We Do | Competitor Gap |
|-------|------------|----------------|
| **Find** | AI-powered swipe matching | LinkedIn is passive; FB groups are noise |
| **Verify** | Portfolio verification + social proof | Behance has no trust signals |
| **Work** | Project workspaces with milestones | Slack/Notion aren't creator-native |
| **Get Paid** | Escrow payments via Stripe Connect | Upwork takes 20%; we take 8-15% |

**The Hook:** "Tinder meets Upwork for creatives"

---

## Slide 4: Product Demo / Screenshots

### The Kretopia Experience

**Circle (AI Matching)**
- Swipe-based discovery with AI match explanations
- Filter by role, skills, location, collaboration intent
- 98% of users upgraded to Pro tier

**Nearby (Local Discovery)**
- Map-based creator discovery
- Creative Sessions (real-world meetups)
- Launching first in Bali creative hub

**ThriveDesk (Project Workspace)**
- Kanban task boards
- Milestone-based payments with escrow
- Real-time messaging and file sharing

**ThrivePay (Payments)**
- Stripe Connect integration
- 8% fee (Pro) / 15% fee (Free)
- Automatic invoice generation

---

## Slide 5: Traction

### Early Metrics That Matter

**Platform Metrics (V3 Relaunch - Feb 2026):**
| Metric | Value | Notes |
|--------|-------|-------|
| **Registered Users** | 53 | Organic signups since relaunch |
| **Pre-Populated Profiles** | 199 | Existing network ready to onboard |
| **Pro Tier Conversion** | Growing | Active subscription testing |
| **Matches Created** | 11 | Early engagement validation |
| **Projects Started** | 8 | End-to-end flow working |

**Distribution Channels Ready to Activate:**
| Channel | Reach | Status |
|---------|-------|--------|
| **Bali WhatsApp Community** | 2,000+ | Active, engaged |
| **Social Media Following** | 6,000+ | Multi-platform |
| **Email List** | 10,000+ | Curated over 13 years |

**What This Proves:**
- ✅ Platform is live and functional
- ✅ Core user journey validated (Match → Project → Payment)
- ✅ 18,000+ person distribution network ready to activate
- ✅ 13 years of community trust = low CAC opportunity

---

## Slide 6: User Composition

### Our Existing Network & Community

**International Industry Professionals:**
- Grammy Award Winners
- Major Label Artists & Producers
- Influencers & Content Creators
- Celebrity Network

**Geographic Expansion Ready:**
| Region | Status |
|--------|--------|
| 🇮🇩 **Bali, Indonesia** | Active launch hub |
| 🇹🇹 **Caribbean** | Strong existing community |
| 🇦🇪 **Dubai** | Established network |
| 🇪🇺 **Europe** | Ready for expansion |
| 🇺🇸 **Los Angeles** | Industry connections |

**Creator Breakdown (Pre-Populated Profiles):**
| Role | Count |
|------|-------|
| Engineers/Mixers | 41 |
| Cinematographers | 38 |
| Producers | 25 |
| Music Producers | 11 |
| Content Creators | 9 |
| Directors/DPs | 8 |
| Other Creatives | 100+ |

**Key Insight:** Cross-industry mix proves the horizontal platform thesis. Audio engineers matching with cinematographers = new collaboration patterns.

---

## Slide 7: The Thrive Ecosystem (Our Unfair Advantage)

### 13 Years of Community Building

**Thrive Collective (Est. 2013):**
| Asset | Scale | Value |
|-------|-------|-------|
| **Events Hosted** | 500+ | Dubai, LA, Geneva, Trinidad, Bali |
| **Creatives Showcased** | 1,000s | Verified track record |
| **Speakers Featured** | 90+ | Industry credibility |
| **Attendees** | 1,000s | Built-in user base |

**Event Brands:**
- **ThriveXchange** - Industry networking
- **Kretopia Bali** - Local creator community
- **Kretopia Dine** - Intimate creator dinners
- **Thrive Master Class** - Educational workshops

**Owned Media Properties:**
| Property | Reach | Status |
|----------|-------|--------|
| **Discover A Thriver Podcast** | Top industry guests | Active |
| **Kretopia Magazine** | 400+ locations in Bali | Bali's top print magazine |

**Why This Matters:**
> Most startups spend years building community trust. We've spent 13 years doing it—now we're digitizing it.

---

## Slide 8: Market Opportunity

### The Creator Economy is Massive and Underserved

**TAM (Total Addressable Market):**
- 50M+ professional creators globally
- $250B+ creator economy by 2027

**SAM (Serviceable Addressable Market):**
- 10M+ creators actively seeking collaboration
- $25B+ in freelance creative work annually

**SOM (Serviceable Obtainable Market):**
- 500K creators in our initial verticals (music, film, content)
- $500M opportunity in Year 1-3

**Why Now:**
1. **Remote collaboration** normalized post-pandemic
2. **Creator economy** exploding (YouTube, TikTok, Spotify)
3. **Fragmentation** - no dominant horizontal platform
4. **AI timing** - match quality finally possible

---

## Slide 9: Product Timeline & Validation

### From Movement to Platform

| Year | Milestone | Validation |
|------|-----------|------------|
| **2013** | Thrive Collective founded | Community-first approach |
| **2020** | Kretopia V1 Launch | 1,000 users acquired |
| **2023** | Kretopia V2 | $50K grant from NEAR Protocol |
| **2026** | Kretopia V3 Relaunch | Full-stack platform with AI matching |

**V3 Platform Features:**
- ✅ AI-powered swipe matching (Circle)
- ✅ Local creator discovery (Nearby)
- ✅ Project workspaces with milestones (ThriveDesk)
- ✅ Escrow payments via Stripe (ThrivePay)
- ✅ Digital marketplace
- ✅ Creative Sessions (real-world meetups)

**Grant Validation:** Received $50K from NEAR Protocol in 2023, demonstrating early institutional confidence in the vision.

---

## Slide 10: Competitive Landscape

### Why Kretopia Wins

| Platform | What They Do | What They Don't |
|----------|--------------|-----------------|
| **LinkedIn** | Professional networking | No creator portfolios, no payments |
| **Behance** | Portfolio showcase | Passive discovery, no matching |
| **Upwork/Fiverr** | Gig marketplace | Race to bottom, 20% fees, no community |
| **Vampr** | Music networking | Music-only, no project tools |
| **Facebook Groups** | Community forums | No verification, no payments |

**Kretopia's Unfair Advantages:**
1. **AI Matching** - Personalized, explainable recommendations
2. **Verification Layer** - IMDB, Spotify, social proof integration
3. **Full Lifecycle** - Find → Verify → Work → Get Paid in one platform
4. **Creator-First UX** - Swipe interface built for mobile-first generation
5. **Community Network Effects** - Each user makes platform more valuable

---

## Slide 11: Business Model

### How We Make Money

**Revenue Streams:**

| Stream | Rate | Status |
|--------|------|--------|
| **Pro Subscriptions** | $12/month | Live - 245 users |
| **Platform Fees (Free)** | 15% of transactions | Live |
| **Platform Fees (Pro)** | 8% of transactions | Live |
| **Digital Marketplace** | 15%/7% commission | Built |

**Unit Economics Target (18 months):**
- CAC: $15-25
- LTV: $150-300
- LTV:CAC Ratio: 6-12x

**Revenue Projections:**
| Year | Subscribers | Transactions | ARR |
|------|-------------|--------------|-----|
| Y1 | 2,000 | $100K GMV | $200K |
| Y2 | 10,000 | $1M GMV | $1.2M |
| Y3 | 50,000 | $10M GMV | $8M |

---

## Slide 12: Go-to-Market Strategy

### Land & Expand (Community-First)

**Phase 1: Activate Existing Network (Now - Month 6)**
- Convert 18,000+ person distribution list
- In-person onboarding via existing events
- Target: 2,500 active users
- Leverage Kretopia Magazine, Podcast, Events

**Phase 2: Caribbean, Dubai, Europe (Month 6-12)**
- Activate existing communities in these regions
- Partner with established venue networks
- Target: 10,000 active users

**Phase 3: Global Expansion (Month 12-18)**
- Los Angeles, London, Lagos
- Industry-specific campaigns (Film festivals, music conferences)
- Target: 50,000 active users

**Growth Channels:**
1. **Owned distribution** - 18K+ email/social/WhatsApp (FREE)
2. **Event integration** - ThriveXchange, Kretopia Dine, Master Class
3. **Media amplification** - Podcast, Magazine, Press
4. **Community-led growth** - Invite system, referral rewards

---

## Slide 13: Press & Credibility

### As Seen In

- **Swagger Magazine**
- **Loop Caribbean**
- **Trinidad Express**

**Industry Recognition:**
- 13 years of consistent community building
- Trusted by Grammy winners and industry professionals
- Established presence across 5+ countries

---

## Slide 14: The Founder

### Ethan Auguste

**21+ Years in Events, Media & Creative Industries**

Ethan Auguste is the Founder of Thrive, a global creative and community-driven ecosystem built to connect, empower, and monetize the world's next generation of creators, entrepreneurs, and cultural leaders.

**Track Record:**
- 🎪 **500+ events produced** across Dubai, Geneva, LA, Trinidad & Bali
- 🎨 **1,000s of creatives showcased** before "creator economy" was a term
- 🌍 **Global network** spanning Grammy winners to emerging talent
- 📱 **Platform builder** - V1 (1K users), V2 ($50K grant), V3 (full-stack)

**Leadership Philosophy:**
> "Culture-first thinking—merging real-world connection with digital infrastructure, building businesses that prioritize community, authenticity, and long-term value over short-term hype."

**Why Ethan Wins:**
- Not a tech founder trying to build community
- A community builder who's learned tech
- 13 years of trust = unfair distribution advantage

**Hiring Plan (Post-Raise):**
- **Full-Stack Engineer** - Scale platform, mobile app
- **Head of Growth** - Community building, partnerships
- **Contractors** - Design, content, support

---

## Slide 15: Use of Funds

### $350K Allocation (18 Months Runway)

| Category | Allocation | Purpose |
|----------|------------|---------|
| **Engineering** | $140K (40%) | Full-stack hire, mobile app, AI improvements |
| **Growth** | $120K (35%) | Marketing, events, partnerships, content |
| **Operations** | $55K (15%) | Infrastructure, tools, legal, admin |
| **Buffer** | $35K (10%) | Unexpected opportunities, market shifts |

**Key Milestones:**
- Month 6: 2,500 active users, $15K MRR
- Month 12: 10,000 users, $75K MRR, seed-ready metrics
- Month 18: 25,000 users, $200K MRR, Series A conversations

---

## Slide 16: The Ask

### Raising $350K+ Pre-Seed

**Terms:**
- **Raise:** $350K+
- **Valuation:** $1.75M - $2M pre-money
- **Instrument:** SAFE or Priced Round (flexible)
- **Use:** 18 months runway to Series A metrics

**Ideal Investors:**
- Angels with creator economy experience
- Funds focused on marketplaces, SaaS, or creator tools
- Strategic partners in music, film, or content industries

**What You Get:**
- Ground floor of the creator economy operating system
- 13 years of community trust = built-in distribution
- 18,000+ person network ready to activate
- Proven founder with 500+ events and global network
- Clear path to $2M+ ARR within 24 months

---

## Slide 17: Vision

### The Future of Creative Work

**Short Term (18 months):**
- Dominant platform for creative collaboration in SEA
- 25K+ active creators, $150K+ MRR
- Series A ready

**Medium Term (3 years):**
- Global platform with 500K+ creators
- $10M+ ARR, profitable
- Expanded to education, equipment rental, studio booking

**Long Term (5+ years):**
- The "LinkedIn for Creators" - professional identity layer
- Creator passport / OAuth provider for the industry
- Acquisitions or IPO path

**The Big Bet:**
> Every creator will need a verified professional identity.
> Kretopia will be that identity.

---

## Slide 18: Contact / Next Steps

### Let's Build the Future of Creative Collaboration

**Ready to discuss:**
- Product demo
- Detailed metrics
- Partnership opportunities

**Contact:**
- Founder: Ethan Auguste
- Website: kretopia.com
- Email: [your email]

---

## Appendix A: Feature Deep Dive

### Circle (AI Matching)
- Swipe-based discovery (Tinder UX)
- AI match explanations ("You both work in music production")
- Filters: Role, skills, location, collaboration intent
- Daily swipe limits (Free: 30, Pro: unlimited)

### Nearby (Local Discovery)
- Map-based creator discovery
- Location privacy (fuzzing for safety)
- Creative Sessions (scheduled meetups)
- Real-time availability indicators

### ThriveDesk (Project Workspace)
- Kanban task boards with drag-and-drop
- Milestone-based project structure
- Real-time messaging
- File sharing and version control
- Timeline and deadline tracking

### ThrivePay (Payments)
- Stripe Connect integration
- Escrow for milestones
- Automatic invoice generation
- Multi-currency support
- Platform fee deduction

### Profile & Verification
- Portfolio showcase with media
- Social stats integration (YouTube, Spotify, Instagram)
- IMDB/Discogs verification
- Reviews and testimonials
- Achievement badges and XP system

---

## Appendix B: Technical Architecture

**Stack:**
- Frontend: React, TypeScript, Tailwind CSS
- Backend: Supabase (Postgres, Edge Functions, Auth)
- Payments: Stripe Connect
- AI: Lovable AI (Gemini/GPT models)
- Hosting: Lovable Cloud (auto-scaling)

**Key Technical Differentiators:**
- Real-time matching algorithm
- Portfolio verification pipeline
- Location-based discovery with privacy
- Scalable serverless architecture

---

## Appendix C: Competitive Research

### Platform Comparison Matrix

| Feature | Kretopia | LinkedIn | Behance | Upwork | Vampr |
|---------|----------|----------|---------|--------|-------|
| Swipe Matching | ✅ | ❌ | ❌ | ❌ | ✅ |
| AI Explanations | ✅ | ❌ | ❌ | ❌ | ❌ |
| Portfolio Verification | ✅ | ❌ | ❌ | ⚠️ | ⚠️ |
| Project Management | ✅ | ❌ | ❌ | ✅ | ❌ |
| Integrated Payments | ✅ | ❌ | ❌ | ✅ | ❌ |
| Local Discovery | ✅ | ❌ | ❌ | ❌ | ❌ |
| Creative Sessions | ✅ | ❌ | ❌ | ❌ | ❌ |
| Cross-Industry | ✅ | ✅ | ⚠️ | ✅ | ❌ |
| Platform Fees | 8-15% | N/A | N/A | 20% | N/A |

---

## Appendix D: Key Risks & Mitigations

| Risk | Likelihood | Impact | Mitigation |
|------|------------|--------|------------|
| **Network Effects Slow** | Medium | High | Focus on dense geographic clusters first (Bali) |
| **Competition** | Medium | Medium | Speed to market + community moat |
| **Payments Complexity** | Low | Medium | Stripe Connect handles compliance |
| **Churn** | Medium | Medium | Focus on habit-forming features (Sessions, messaging) |
| **Solo Founder Risk** | Medium | High | Plan to hire quickly post-raise |

---

## Appendix E: Monthly Burn Rate

**Target Monthly Burn: $19K**

| Category | Monthly |
|----------|---------|
| Engineering (hire) | $8,000 |
| Growth (marketing + events) | $6,500 |
| Infrastructure | $1,500 |
| Tools & Subscriptions | $1,000 |
| Legal & Admin | $1,000 |
| Buffer | $1,000 |
| **Total** | $19,000 |

**Runway:** 18.4 months at $19K/month burn

---

*End of Deck*

**Questions? Let's talk:** [your email]
